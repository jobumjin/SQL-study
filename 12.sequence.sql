...
2. 대표적인 활용 영역
	...
	- 게시글 table의 어떤 컬럼에 적용할지 먼저 분석, 설계후 생성
	- 주의 사항
		: 시퀀스는 여러 table에서 송유 금지
...
--1. sequence 생성 명령어
-- a 라는 table의 no는 고유한 숫자값 (counting)으로 간주
drop table a;
drop table b;
...
-- table별 고유한 숫자 사용 불가문제 발생 왜? 공유되기 때문에
insert into a values (a_no_seq.nextval, 'db');
