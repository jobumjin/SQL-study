...
- insert/update/delete 문장의 필수 항목
	...
	: db 자체에는 임시 저장소와 영구 저장소로 분리
		insert/update/delete 작업은 commit 명령어 전까지는 임시 저장소에 저장
	...
	- 트랜젝션
		...
		참고 : dbeaver 는 기본이 auto commit 기능 보유 
*/
...
-- people table은 제약조건 무
-- 컬럼 두개 모두 null 허용
-- 하나의 컬럼에만 데이터 insert해서 새로운 row 생성 의미
create table people(
	name varchar2(10),
	age number(3)
);
...
--? 11, 유재석, 10 emp01에 /22, 백종원, 20 emp02에
insert all
...
select comm, deptno from emp01 where comm is not null;
...
-- ? 문법 이해를 위한 문제 분석해 보기
/* produce_total로 생산된 데이터들을 insert 또는 update
1. 새로운 제품 - insert
