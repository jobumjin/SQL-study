...
-- 반올림자리수 : 소수점을 기준으로 양수는 소수점 이하 자리수 의미
--               음수인 경우 정수자리수를 의미
...
select empno, round(sysdate-hiredate) from emp;
...
--4. ? emp table 입사일 이후 3개월 지난 일wk 검색
...
select months_between(sysdate, '20211224') from dual;
...
select last_day('200201') from dual;
...
-- select sysdate - 20200719 from dual; 오류
-- select sysdate - '20200719' from dual; 오류
...
-- [3] to_number(문자열 ,변환 포맷) : 문자열을 숫자로 변환
